...
class TestDetectionScope:
    ...
    def test_scope_events_hard_rule(self):
        ...
        # (NORMAL ECG / NORMAL SINUS RHYTHM removed from scope 2026-06-01)
        ...
        assert not event_in_scope("Normal ECG")
        assert not event_in_scope("Normal Sinus Rhythm")
        assert not event_in_scope("Isolated Ventricular Beat")

    def test_normal_heads_out_of_scope(self):
        # NORMAL ECG (2) and NORMAL SINUS RHYTHM (1) are no longer detection heads
        probs = [0.0] * 150
        probs[2] = 0.9
        probs[1] = 0.9
        assert detect(probs, "Normal ECG") is None       # removed from scope
        assert detect_index(probs, 2) is None
        assert detect_index(probs, 1) is None
        assert not in_scope(1) and not in_scope(2)
    ...
    def test_scope_consistency_invariant_holds(self):
        ...
        # all 6 scope events are fzark events; NON_FZARK_SCOPE_EVENTS is empty
        assert NON_FZARK_SCOPE_EVENTS == frozenset()
        assert "Normal ECG" not in scope_events()
